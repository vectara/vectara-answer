...
if [ -z "$correct_path" ]; then
  docker run --platform linux/arm64 -d --env-file .env --name vanswer -p 80:3000/tcp vectara-answer
else
  docker run --platform linux/arm64 -d -v "$correct_path":/usr/src/app/extras/ --env-file .env --name vanswer -p 80:3000/tcp vectara-answer
fi

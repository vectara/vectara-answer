...
`python3 docker/prepare_config.py /path/to/<YOUR-CONFIG-FOLDER> <profile-name>`

Usually the `profile-name` is called `default`, but if you have multiple profiles in your `secrets.toml` file then you need to pick the profile that you need from that file.
